...
// --- EXPORT & IMPORT LOGIC (TEACHER/STUDENT SHARING) ---
window.exportCurrentSession = () => {
    if (chatHistory.length === 0) {
        alert("There is no conversation to export. Please ask a question first.");
        return;
    }
    
    // Package the data
    const sessionData = {
        title: currentSessionTitle || `Lesson: ${new Date().toLocaleDateString()}`,
        messages: chatHistory,
        libraryItem: selectedLibraryItem // Saves the book they were referencing
    };
    ...
    downloadAnchorNode.setAttribute("download", `Eprashala_Lesson_${Date.now()}.json`);
    ...
    reader.onload = async (e) => {
        try {
            ...
            if (importedData.libraryItem && importedData.libraryItem.includes('|')) {
                ...
                UI.ddText.innerText = selectedLibraryItem.split('|')[1];
            }

            UI.settingsModal.classList.add('hidden'); // Close modal so they can see the chat
...
function setupEventListeners() {
    ...
    if (UI.btnImport) {
        UI.btnImport.addEventListener('click', (e) => {
            e.stopPropagation();
            UI.fileImport.click(); // Triggers the hidden file input
        });
    }
...
function getDhwaniGreeting(langCode, author, book) {
    ...
    switch (lang) {
        ...
        case 'mr':
            return `नमस्कार, मी ध्वनी. मी तुमची कृत्रिम बुद्धिमत्तेवर आधारित ज्ञानमार्गदर्शक आहे. कसे आहात तुम्ही? मी निवडलेल्या प्राचीन ग्रंथांच्या आधारे तुमच्या प्रश्नांची उत्तरे देत आहे.`;

        case 'hi':
            return `नमस्कार, मैं ध्वनि हूँ। मैं आपकी कृत्रिम बुद्धिमत्ता आधारित ज्ञान मार्गदर्शक हूँ। आप कैसे हैं? मैं चुने गए प्राचीन ग्रंथों के आधार पर आपके प्रश्नों के उत्तर दे रही हूँ।`;

        case 'en':
            return `Hello, I am Dhwani. I am your AI Knowledge Guide. How are you? I will answer your questions based on the selected ancient scriptures.`;

        case 'bn':
            return `নমস্কার, আমি ধ্বনি। আমি আপনার কৃত্রিম বুদ্ধিমত্তাভিত্তিক জ্ঞান-সহায়ক। আপনি কেমন আছেন? আমি নির্বাচিত প্রাচীন গ্রন্থগুলির ভিত্তিতে আপনার প্রশ্নগুলির উত্তর দিচ্ছি।`;

        case 'te':
            return `నమస్కారం, నేను ధ్వని. నేను మీ కృత్రిమ మేధస్సు ఆధారిత జ్ఞాన మార్గదర్శిని. మీరు ఎలా ఉన్నారు? ఎంపిక చేసిన ప్రాచీన గ్రంథాల ఆధారంగా మీ ప్రశ్నలకు నేను సమాధానం ఇస్తాను.`;

        case 'ta':
            return `வணக்கம், நான் த்வனி. நான் உங்கள் செயற்கை நுண்ணறிவு அறிவு வழிகாட்டி. நீங்கள் எப்படி இருக்கிறீர்கள்? தேர்ந்தெடுக்கப்பட்ட பண்டைய நூல்களின் அடிப்படையில் உங்கள் கேள்விகளுக்கு நான் பதிலளிக்கிறேன்.`;

        case 'gu':
            return `નમસ્કાર, હું ધ્વનિ છું. હું તમારી કૃત્રિમ બુદ્ધિમત્તા આધારિત જ્ઞાન માર્ગદર્શિકા છું. તમે કેમ છો? હું પસંદ કરેલા પ્રાચીન ગ્રંથોના આધારે તમારા પ્રશ્નોના જવાબ આપી રહી છું.`;

        case 'kn':
            return `ನಮಸ್ಕಾರ, ನಾನು ಧ್ವನಿ. ನಾನು ನಿಮ್ಮ ಕೃತಕ ಬುದ್ಧಿಮತ್ತೆ ಆಧಾರಿತ ಜ್ಞಾನ ಮಾರ್ಗದರ್ಶಿ. ನೀವು ಹೇಗಿದ್ದೀರಿ? ಆಯ್ಕೆ ಮಾಡಿದ ಪ್ರಾಚೀನ ಗ್ರಂಥಗಳ ಆಧಾರದ ಮೇಲೆ ನಿಮ್ಮ ಪ್ರಶ್ನೆಗಳಿಗೆ ನಾನು ಉತ್ತರಿಸುತ್ತೇನೆ.`;

        case 'ml':
            return `നമസ്കാരം, ഞാൻ ധ്വനി. ഞാൻ നിങ്ങളുടെ കൃത്രിമ ബുദ്ധി അധിഷ്ഠിത വിജ്ഞാന മാർഗദർശിയാണ്. നിങ്ങൾക്ക് സുഖമാണോ? തിരഞ്ഞെടുത്ത പ്രാചീന ഗ്രന്ഥങ്ങളുടെ അടിസ്ഥാനത്തിൽ നിങ്ങളുടെ ചോദ്യങ്ങൾക്ക് ഞാൻ മറുപടി നൽകും.`;

        case 'pa':
            return `ਸਤ ਸ੍ਰੀ ਅਕਾਲ, ਮੈਂ ਧਵਨੀ ਹਾਂ। ਮੈਂ ਤੁਹਾਡੀ ਕ੍ਰਿਤ੍ਰਿਮ ਬੁੱਧੀ ਆਧਾਰਿਤ ਗਿਆਨ ਮਾਰਗਦਰਸ਼ਕ ਹਾਂ। ਤੁਸੀਂ ਕਿਵੇਂ ਹੋ? ਮੈਂ ਚੁਣੇ ਹੋਏ ਪ੍ਰਾਚੀਨ ਗ੍ਰੰਥਾਂ ਦੇ ਆਧਾਰ 'ਤੇ ਤੁਹਾਡੇ ਸਵਾਲਾਂ ਦੇ ਜਵਾਬ ਦਿਆਂਗੀ।`;

        case 'or':
            return `ନମସ୍କାର, ମୁଁ ଧ୍ୱନି। ମୁଁ ଆପଣଙ୍କ କୃତ୍ରିମ ବୁଦ୍ଧିମତ୍ତା ଆଧାରିତ ଜ୍ଞାନ ମାର୍ଗଦର୍ଶିକା। ଆପଣ କେମିତି ଅଛନ୍ତି? ଚୟନ କରାଯାଇଥିବା ପ୍ରାଚୀନ ଗ୍ରନ୍ଥମାନଙ୍କ ଆଧାରରେ ମୁଁ ଆପଣଙ୍କ ପ୍ରଶ୍ନର ଉତ୍ତର ଦେବି।`;
