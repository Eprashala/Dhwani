...
let selectedLibraryItem = "12 Adityas (Solar Deities)|Mitra";
...
async function loadLibraryConfig() {
    try {
        const response = await fetch('library_deity.json');
...
function renderDropdownList(filterText = "") {
    ...
    for (const groupName in LIBRARY_CONFIG) {
        ...
        groupDiv.innerHTML = `<div class="text-[14px] uppercase text-cyan-600 font-bold px-3 py-1.5 mt-1 bg-slate-900 sticky top-0 z-10 shadow-sm">${groupName}</div>`;
