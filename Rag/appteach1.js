...
// --- SILENT Q&A LOGGING ---
function logQAToServer(userQuery, botReply) {
    // Determine which app is sending the data
    const isLibrary = document.title.includes("Library");
    let contextData = {};
    
    if (isLibrary) {
        contextData = {
            book_title: typeof selectedLibraryItem !== 'undefined' ? selectedLibraryItem : "General"
        };
    } else {
        contextData = {
            medium: document.getElementById('medium-selector') ? document.getElementById('medium-selector').value : "Unknown",
            standard: document.getElementById('std-selector') ? document.getElementById('std-selector').value : "Unknown",
            subject: document.getElementById('subject-selector') ? document.getElementById('subject-selector').value : "Unknown"
        };
    }

    const payload = {
        app: isLibrary ? "library" : "teacher",
        timestamp: new Date().toISOString(),
        context: contextData,
        query: userQuery.trim(),
        response: botReply.trim()
    };

    // Fire-and-forget network request
    fetch("https://ai.eprashala.com/api/log_qa", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
async function processInput(userText, isHiddenQuizTrigger = false) {
	...
	try {
        ...
        updateEditPencil();
		logQAToServer(userText, displayRes);
